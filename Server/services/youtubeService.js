...
class YouTubeService {
	...
	async processVideo(videoUrl, videoId, userId = null) {
		try {
			...
			const lesson = this.buildLessonObject(
				videoUrl,
				videoId,
				videoData,
				learningContent,
	...
	async generateLearningContent(videoUrl, videoData) {
		const prompt = YouTubePromptBuilder.createYouTubeLearningPrompt(
			videoUrl,
			videoData
		);
	...
	getFallbackLearningContent() {
		return {
			...
			summary:
				"This video contains educational material that can help you learn new concepts and skills.",
			notes: [
